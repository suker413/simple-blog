...
env.user = 'moling'
env.sudo_user = 'root'
env.hosts = ['119.29.238.149']

db_user = 'simpleblog'
db_password = 'test'

_TAR_FILE = 'dist-simpleblog.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/simpleblog'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-simpleblog-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick simpleblog > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop simpleblog')
        sudo('supervisorctl start simpleblog')
...
def restore2local():
    ...
    sqls = [
        ...
        'grant select, insert, update, delete on simpleblog.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s simpleblog < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
